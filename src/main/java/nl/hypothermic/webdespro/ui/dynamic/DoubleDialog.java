...
public class DoubleDialog implements Initializable {
	...
	private List<IDynListener> listeners = new ArrayList<IDynListener>();

    public void addListener(IDynListener listener) {
	...
	@FXML private void onClosed() {
		for (IDynListener l : listeners) {
			l.onClose(new String[] {prop.getText(), val.getText()});
		}
	}
